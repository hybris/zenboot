...
TMPFILE=`tempfile`
CALLBACK=`curl -sL  --write-out '%{http_code}' --request POST --max-time 5 --basic --user 'sanitycheck:sanitycheck' -H 'Content-Type:text/xml' http://127.0.0.1:8080/zenboot/rest/sanitycheck | sed "s/.*<referral>//" | sed "s/<\/referral>.*//"`
